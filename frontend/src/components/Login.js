...
const Login = () => {
  ...
  const handleRegisterPopup = async () => {
    ...
    if (formValues) {
      try {
        await axios.post('https://ai-event-planner-backend.onrender.com/api/v1/auth/signup', formValues);
        Swal.fire({ 
          icon: 'success', 
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://ai-event-planner-backend.onrender.com/api/v1/auth/login', { email, password });
