...
const Hero = () => {
  ...
  useEffect(() => {
    const getRecommendations = async () => {
      try {
        ...
        const res = await axios.get(`https://ai-expense-event-tracker.onrender.com/api/v1/recommendations?userId=${userId}`, {
  ...
  const handleSearch = async (e) => {
    ...
    if (value.length > 1) {
      try {
        const res = await axios.get(`https://ai-expense-event-tracker.onrender.com/api/v1/search?query=${value}`);
